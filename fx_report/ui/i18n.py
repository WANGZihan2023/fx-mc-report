...
_STRINGS: dict[str, dict[str, str]] = {
    LANG_ZH: {
        ...
        "dlg.simple.caption": "简洁模式：只需货币对 + 看涨货币（可上传单子 PDF）。算法由系统按校准 JSON → 引擎对比 → 产品默认推荐；确认后可在侧栏点「改用专家设置」。",
        "dlg.expert.caption": "专家模式：请逐项选择峰值引擎 / 校准 / 人工确认；不预选默认项。也可上传单子 PDF 自动填入货币对与看涨；算法项仍须手选。",
        "dlg.upload_pdf": "上传单子 PDF（可选）",
        "dlg.upload_pdf.label": "上传单子 PDF",
        "dlg.upload_pdf.help": "识别货币对、看涨方向、Barrier/Strike/分档等；识别失败可继续手动填。",
    ...
    LANG_EN: {
        ...
        "dlg.simple.caption": "Simple mode: only pair + bullish currency (optional order PDF). Algorithms are recommended from calib JSON → engine compare → product defaults. Switch to Expert from the sidebar afterward.",
        "dlg.expert.caption": "Expert mode: pick peak engine / calibration / human review yourself (no pre-selection). Order PDF can still fill pair + bullish; algorithm fields stay manual.",
        "dlg.upload_pdf": "Upload order PDF (optional)",
        "dlg.upload_pdf.label": "Upload order PDF",
        "dlg.upload_pdf.help": "Detects pair, bullish side, Barrier/Strike/buckets; on failure you can fill manually.",
        "dlg.pdf.parsing": "Parsing order…",
        "dlg.pdf.fail": "Could not parse the PDF—please fill manually.",
        "dlg.pdf.ok": "PDF parsed (please verify prefilled fields; unrecognized items still need a manual choice).",
