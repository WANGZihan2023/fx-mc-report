...
2. 若出现 **「人工确认不确定证据」**，先选利多/利空/中性/跳过再继续（见 `docs/human_review.md`）  
3. 在 **「本次分析审计」正下方** 找到 **「证据人工标注」**（完整报告之上；也可开侧栏 **⑨ 证据人工标注**）  
4. 展开 **「怎么填？」** 看例子  
5. （可选）点 **「一键按模型预填再改」**，只改不同意的几条  
6. 每条选 `human_direction` / `human_category`；`agree` 一般会自动填  
7. 有标注后页面会显示 **抽检准确率**（= 同意率 yes / yes+no）；练习样例会标明非正式  
8. （可选）勾选 **「用人工标注重算权重」** → **「应用人工标注重算」**：用人工方向覆盖模型方向，重算 S 与情景权重（并尽量重跑 MC）  
9. 点 **「保存标注到 output/」** → 文件  
